...
def compare_images(original_path, tampered_path, output_folder="tamper_results"):
    ...
    try:
        ...
        comparison = np.hstack((original, tampered))
        ...
        changed_coords = cv2.findNonZero(ssim_thresh)
        if changed_coords is not None:
            x, y, w, h = cv2.boundingRect(changed_coords)
            cropped_changes = ssim_thresh[y:y+h, x:x+w]
        else:
            cropped_changes = ssim_thresh  # fallback: entire image
        
        # Step 8: Compute absolute difference
        error_map = cv2.absdiff(original, tampered)

        #  Multiply by 10 to enhance the visibility
        ...
        cv2.imwrite(os.path.join(output_folder, "03_comparison_input.png"), comparison)
        cv2.imwrite(os.path.join(output_folder, "04_diff_gray.png"), diff_gray)
        cv2.imwrite(os.path.join(output_folder, "05_diff_rgb.png"), diff_rgb)
        cv2.imwrite(os.path.join(output_folder, "06_delta_e_gray.png"), delta_e_norm)
        cv2.imwrite(os.path.join(output_folder, "07_delta_e_colormap.png"), delta_e_color)
        cv2.imwrite(os.path.join(output_folder, "08_highlighted_diff.png"), highlight)
        cv2.imwrite(os.path.join(output_folder, "09_ssim_thresh.png"), ssim_thresh)
        cv2.imwrite(os.path.join(output_folder, "10_final_binary_output.png"), final_binary)
        cv2.imwrite(os.path.join(output_folder, "11_cropped_changed_region.png"), cropped_changes)
        cv2.imwrite(os.path.join(output_folder, "12_colored_error_map_x10.png"), error_colored)
